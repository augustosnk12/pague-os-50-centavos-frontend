...
- **Status is derived, not stored:** `instStatus(inst)` computes `PAID | PARTIAL | OVERDUE | PENDING` from `paidAmount` + `dueDate` vs. today. `PAID` when `paidAmount >= amount`; `PARTIAL` when `0 < paidAmount < amount`; otherwise date-based. The API also returns a `status` field — prefer the API's value, but the derivation logic is a useful fallback/cross-check.
- **Partial payments:** each installment carries `paidAmount` (cumulative) **and** a `payments: [{ id, amount, date }]` history array. `instPaid`/`instRemaining`/`instPayments` (in `data.jsx`) are the accessors. Backend needs a payments/transactions table (one row per payment) with the installment's `paidAmount` = sum; "mark paid" becomes "register payment" (`POST /installments/:id/payments` or similar). All money aggregations (dashboard totals, debtor outstanding, period totals) sum **remaining** = `amount − paidAmount`, not full amounts.
...
| `PARTIAL` (Parcial) | partially paid | `oklch(0.55 0.13 250)` blue | `oklch(0.95 0.04 250)` |
...
Left status-colored 4px border · avatar (→ debtor) · debtor name + "Parcela {n}" (+ a **Parcial** badge when partially paid) · context line ("Vence em N dias · {date}" / "Atrasado há N dias" / "Vence hoje" / "Pago em {date}" / "{paid} pago · falta {remaining}") · amount (for PARTIAL: the **remaining** in the partial/blue color with a small "de {full}") · a filled-**primary** **Receber** button **or** a Pago badge. Receber opens the **PaymentSheet** (it does NOT mark paid in one tap — that was the old behavior).

### Shared: `PaymentSheet` (`app/payment.jsx`) — register a payment (full or partial)
Opened by any **Receber** action. Shows the debtor/installment context, a balance summary (**Valor da parcela / Já pago / Restante**), then:
- **Valor recebido** input (defaults to the full remaining) with quick chips **Metade** (remaining/2) and **Total** (remaining).
- **Data do pagamento** (defaults to today).
- A live result line: "Pagamento parcial · Restante após: {x}" or "Parcela será quitada".
- **Histórico de pagamentos** — a collapsible accordion (collapsed by default; header shows count + total received) listing each prior payment (date + amount). For an already-fully-paid installment the sheet becomes **read-only** (history only, defaults expanded, title "Pagamentos da parcela") — reachable via the "{n} pagamentos" chip on paid rows in the debt card.
- Confirm → register payment: append to `payments[]`, recompute `paidAmount`; toast "Pagamento de {x} registrado" / "Parcela quitada". Validates `0 < value ≤ remaining` (no overpay).

### Calculator (`app/calculator.jsx`) — `CalculatorSheet`
A popup calculator (top-bar icon, the Conta sheet, and the drawer all open it). Full keypad (`+ − × ÷ = , C ⌫ ± %`), pt-BR formatted display with a history line, keyboard support (Enter/Esc/Backspace), divide-by-zero → "Erro". Themed with the design tokens. Has an unused `onUse(value)` hook to optionally feed a result into an amount field later.

---

## Interactions & Behavior
- **Register payment (full or partial):** the **Receber** button opens the PaymentSheet (it no longer marks paid in one tap). Confirming appends to `payments[]` and recomputes `paidAmount`; status becomes PARTIAL or PAID. Optimistic update + toast. Backend: `POST` a payment row; on conflict/overpay reconcile.
...
- `prototype/app/payment.jsx` — `PaymentSheet` (register payment + history accordion), `parseAmount`.
- `prototype/app/calculator.jsx` — `Calculator`, `CalculatorSheet`.
